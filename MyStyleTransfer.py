...
def load_and_process_img(path_to_img): #combined functions
    max_dim = 512
...
def get_content_loss(content, input): #loss function number 1
  return tf.reduce_mean(tf.square(content - input))


def gram_matrix(input):
    channels = int(input.shape[-1])
    a = tf.reshape(input, [-1, channels]) #flatten
    n = tf.shape(a)[0]
...
def get_feature_representations(model, content_path, style_path):
    ...
    model_outputs = model(stack_images) #this is the big mocel call
